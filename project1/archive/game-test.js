$( document ).ready( function() {
  ...
  function neighborSum (x, y)  {
    ...
    if ((y === 0) && (x === 0)){
      sum =  b[y+1][x+1]+ b[y+1][x] + b[y][x+1];
    } else if ((y === yBound) && (x === xBound)){
      sum = b[y-1][x]+ b[y-1][x-1] + b[y][x-1];
    } else if ((y === 0) && (x === xBound)){
      sum = b[y][x-1]+ b[y+1][x-1] + b[y+1][x];
    } else if ((y === yBound) && (x === 0)){
      sum = b[y-1][x]+ b[y-1][x+1] + b[y][x+1];
    } else if (y === yBound){
      sum = b[y][x+1] + b[y][x-1] + b[y-1][x+1] + b[y-1][x-1] + b[y-1][x];
    } else if (x === xBound){
      sum = b[y+1][x] + b[y-1][x] + b[y+1][x-1] + b[y][x-1] + b[y-1][x-1];
    } else if (y === 0){
      sum = b[y][x+1] + b[y][x-1] + b[y+1][x+1] + b[y+1][x] + b[y+1][x-1];
    } else if (x === 0){
      sum = b[y+1][x] + b[y-1][x] + b[y+1][x+1] + b[y][x+1] + b[y-1][x+1];
    } else {
      sum = b[y+1][x] + b[y-1][x] + b[y][x+1] + b[y][x-1] + b[y+1][x+1] + b[y-1][x-1]+ b[y+1][x-1]+ b[y-1][x+1];
...
function initMove() {
  ...
  function turnPlr(dir){
    ...
    $('.player').css({transform: 'rotate(' + player.angle + 'deg)'}, 100);
  }
...
function collisionDetect () {
  ...
  touchCells += board[Math.floor(boardY)][Math.floor(boardX)];
  touchCells += board[Math.ceil(boardY)][Math.ceil(boardX)];
  touchCells += board[Math.floor(boardY)][Math.ceil(boardX)];
  touchCells += board[Math.ceil(boardY)][Math.floor(boardX)];
  touchCells += board[Math.floor(boardY+playerMax)][Math.floor(boardX+playerMax)];
  touchCells += board[Math.ceil(boardY+playerMax)][Math.ceil(boardX+playerMax)];
  touchCells += board[Math.floor(boardY+playerMax)][Math.ceil(boardX+playerMax)];
  touchCells += board[Math.ceil(boardY+playerMax)][Math.floor(boardX+playerMax)];
  if (touchCells > 0) {
    injurePlayer();
  }
}
...
function moveProj(projID){
  ...
  // if the projectile has been around for too long, destroy it.
  if (projectiles[projID].decay === 0) {
    $(projID).remove();
    delete projectiles[projID];
    return true;
  };
  // otherwise, move it at the set vector
    projectiles[projID].decay --;
  $(projID).animate({
...
// enemy moves randomly, bounces off walls
function enemyMove() {

}

// enemy drops active cells, seeding the board
function enemyDrop () {

}

// starts movement and collision handlers
initMove();
